(function ($) {
    ...
    $('.btnModuloEstatistico').on('click', function (e) {
        var link = $(".btnModuloEstatistico").data("link-estatistico");
        window.open(link, '_blank');
    });

    $('.EmConstrucao').on('click', function (e) {
        ExibirMensagemGritter('Oops!', 'Página ainda em construção...', 'gritter-warning');
    });
...
function ExibirMensagemGritterTodos(content, msgErroNaoIdentificado, exibirSucesso) {
    if (content.sucesso) {
        if (exibirSucesso && exibirSucesso === true)
            ExibirMensagemGritter('Sucesso!', content.sucesso, 'gritter-success');
        else
            return;
    }
    if (content.alerta)
        ExibirMensagemGritter('Sucesso parcial', content.alerta, 'gritter-warning')
